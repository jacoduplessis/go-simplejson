...
func TestSimplejson(t *testing.T) {
	...
	ms := js.Get("test").Get("string").GetString()
	assert.Equal(t, "simplejson", ms)

	ms2 := js.Get("test").Get("missing_string").GetString("fyea")
	assert.Equal(t, "fyea", ms2)

	ma2 := js.Get("test").Get("missing_array").GetArray()
	assert.Equal(t, []*Json{}, ma2)

	msa := js.Get("test").Get("string_array").GetStringArray()
	assert.Equal(t, msa[0], "asdf")
	assert.Equal(t, msa[1], "ghjk")
	assert.Equal(t, msa[2], "zxcv")

	msa2 := js.Get("test").Get("string_array").GetStringArray([]string{"1", "2", "3"})
	assert.Equal(t, msa2[0], "asdf")
	assert.Equal(t, msa2[1], "ghjk")
	assert.Equal(t, msa2[2], "zxcv")

	msa3 := js.Get("test").Get("missing_array").GetStringArray([]string{"1", "2", "3"})
	assert.Equal(t, msa3, []string{"1", "2", "3"})

	mm2 := js.Get("test").Get("missing_map").GetMap()
	assert.Equal(t, map[string]*Json{}, mm2)
	...
	assert.Equal(t, js.Get("test").Get("bool").GetBool(), true)

	js.Set("float2", 300.0)
	assert.Equal(t, js.Get("float2").GetFloat64(), 300.0)

	js.Set("test2", "setTest")
	assert.Equal(t, "setTest", js.Get("test2").GetString())

	js.Del("test2")
	assert.NotEqual(t, "setTest", js.Get("test2").GetString())

	js.Get("test").Get("sub_obj").Set("a", 2)
	assert.Equal(t, 2, js.Get("test").Get("sub_obj").Get("a").GetInt())

	js.GetPath("test", "sub_obj").Set("a", 3)
	assert.Equal(t, 3, js.GetPath("test", "sub_obj", "a").GetInt())
}
...
func TestSetPathNoPath(t *testing.T) {
	...
	f := js.GetPath("some_number").GetFloat64(99.0)
...
func TestNewFromReader(t *testing.T) {
	...
	ma := js.Get("test").Get("array").GetArray()
	assert.Equal(t, ma, []interface{}{json.Number("1"), "2", json.Number("3")})

	mm := js.Get("test").Get("arraywithsubs").GetIndex(0).GetMap()
	assert.Equal(t, mm, map[string]interface{}{"subkeyone": json.Number("1")})

	assert.Equal(t, js.Get("test").Get("bignum").GetInt64(), int64(9223372036854775807))
	assert.Equal(t, js.Get("test").Get("uint64").GetUint64(), uint64(18446744073709551615))
}
